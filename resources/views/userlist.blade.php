                ...
                <div class="box no-border">
                    <div class="box-header">
                        <h3 class="box-title">All User <label
                                    class="badge">{{\App\User::all()->count()}}</label></h3>
                    </div>
                    <div class="box-body">
                        <table id="mytable" class="table table-bordered table-striped" cellspacing="0" width="100%">
                            ...
                            @foreach($data as $d)
                                <tr>
                                    <td>{{$d->name}} @if(Auth::user()->id == $d->id)
                                            <small class="label pull-right bg-green">You</small> @endif </td>
                                    <td>{{$d->email}}</td>
                                    <td>{{$d->type}}</td>
                                    ...
                                    <td>
                                        ...
                                        @endif
                                        @if(\App\Http\Controllers\Data::hasPackage($d->id,'pack1'))
                                            <i class="fa fa-reddit"></i>
                                        @endif
                                    </td>
                                    <td>

                                        <button data-id="{{$d->id}}" class="btn btn-xs action btn-danger"><i
