...
class LaraUpdaterController extends Controller
{
    ...
    */
    public function update()
    {
        ...
        $data = "";
        $data .= "<h2>SocialLit Software Update</h2>";
        $data .= '<h4><a href="' . url('/') . '">Return to App HOME</a></h4>';
        ...
        if ($lastVersionInfo['version'] <= $this->getCurrentVersion()) {
            $data .= '<p>&raquo; Your System IS ALREADY UPDATED to last version !</p>';
            exit;
        }
        ...
        try {
            ...
            $data .= '<p>UPDATE FOUND: ' . $lastVersionInfo['version'] . ' <i>(current version: ' . $this->getCurrentVersion() . ')</i></p>';
            $data .= '<p>DESCRIPTION: <i>' . $lastVersionInfo['description'] . '</i></p>';
            $data .= '<p>&raquo; Update, downloading ........... ';
            ...
            Artisan::call('down');
            $data .= '<p>&raquo; SYSTEM Mantence Mode => ON</p>';
            ...
            if ($status) {
                ...
                $data .= '<p>&raquo; SYSTEM Mantence Mode => OFF</p>';
                $data .= '<p class="success">SYSTEM IS NOW UPDATED TO VERSION: ' . $lastVersionInfo['version'] . '</p>';
            } else
                throw new \Exception("Error during updating.");

        } catch (\Exception $e) {
            $data .= '<p>ERROR DURING UPDATE (!!check the update archive!!) --TRY to restore OLD status ........... ';

            $this->restore();

            $data .= '</p>';
        }
        return view('softwareUpdate.update',compact('data'));
    }
    ...
    public function check()
    {
        ...
        if (version_compare($lastVersionInfo['version'], $this->getCurrentVersion(), ">"))
            return response()->json([
                ...
                'description'=>$lastVersionInfo['description']
            ]);
